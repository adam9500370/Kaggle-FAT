"""
https://github.com/AnjieZheng/MobileNetV3-PyTorch
"""
...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, name, n_classes=1000, width_mult=1., in_channels=3, dropout_rate=0.5):
        ...
        input_channel = _make_divisible(16 * width_mult, 8) if width_mult > 1.0 else 16
        self.conv0 = conv_3x3_bn(in_channels, input_channel, 2, nl='HS')
        layers = []
        # building inverted residual blocks
        block = InvertedResidual
        # for t, c, n, s in self.cfgs:
        for kernel, expansion, output_channel, se, nl, stride in self.cfg[self.name]:
            layers.append(block(input_channel, output_channel, kernel, stride, expansion, nl=nl, with_se=se))
            input_channel = output_channel
        self.features = nn.Sequential(*layers)

        # building last several layers
        self.conv1 = conv_1x1_bn(input_channel, expansion, nl='HS', with_se=False if 'L' in self.name else True)
            ...
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    if 'classifier' in name:
                        classifier_init_bias = - math.log(self.n_classes - 1.)
                        m.bias.data.fill_(classifier_init_bias)
                        print('classifier sigmoid init bias: {}'.format(classifier_init_bias))
                    else:
                        m.bias.data.zero_()
