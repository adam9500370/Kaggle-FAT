...
#         matrix_a += regularization_weight * batch_identity_matrix
    ...
    ones = torch.ones(1, dtype=train_points.dtype, device=train_points.device).view([-1, 1, 1])
...
#     print('Num_B_Cols', matrix_b.shape)
    ...
    lhs_zeros = torch.randn((b, num_b_cols, num_b_cols), device=train_points.device) / 1e10
...
#     print('LHS', lhs, lhs.shape)
    ...
    rhs_zeros = torch.zeros((b, d + 1, k), dtype=train_points.dtype, device=train_points.device).float()
...
#     print('stacked', stacked_grid.shape)
    ...
    batched_grid = stacked_grid.unsqueeze(-1).permute(3, 1, 0, 2).to(flow.device)
...
#     print('Squeezed query_points', unstacked_query_points[0].shape, unstacked_query_points[1].shape)
    ...
    for dim in index_order:
        ...
        max_floor = torch.tensor(size_in_indexing_dimension - 2, dtype=query_type, device=query_points.device)
        min_floor = torch.tensor(0.0, dtype=query_type, device=query_points.device)
        ...
        alpha = torch.tensor(queries - floor, dtype=grid_type, device=grid.device)
        min_alpha = torch.tensor(0.0, dtype=grid_type, device=grid.device)
        max_alpha = torch.tensor(1.0, dtype=grid_type, device=grid.device)
    ...
    batch_offsets = torch.reshape(
      torch.arange(batch_size, device=query_points.device) * height * width, [batch_size, 1])
